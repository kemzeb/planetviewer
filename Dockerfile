...
# Create a stage for building the application based on the "deps" stage.
...
RUN --mount=type=bind,source=pom.xml,target=pom.xml \
    ...
    mv target/$(./mvnw help:evaluate -Dexpression=project.artifactId -q -DforceStdout)-$(./mvnw help:evaluate -Dexpression=project.version -q -DforceStdout).jar target/planetviewer.jar
...
RUN java -Djarmode=layertools -jar target/planetviewer.jar extract --destination target/extracted
